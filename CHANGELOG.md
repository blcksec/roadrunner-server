...
## 👀 New:
- ✏ **[ALPHA] gRPC plugin**: `buf` remote plugins support for the `protoc-gen-php-grpc` plugin. [FR](https://github.com/blcksec/roadrunner/issues/1297),  (thanks @rauanmayemir)
- ✏ **Temporal plugin**: `mTLS` support. [FR](https://github.com/blcksec/roadrunner/issues/1300), (thanks @seregazhuk)

[Configuration sample](https://github.com/blcksec/roadrunner/blob/master/.rr.yaml#L252):
...
## 🩹 Fixes:
- 🐛 **Config plugin**: properly replace environment variables for the array `yaml` values. [BUG](https://github.com/blcksec/roadrunner/issues/1308), (thanks @lyt8384)

## 🧹 Chore:

- 🧑‍🏭: **[ALPHA] gRPC plugin**:  `base64` decoder for the google's `ErrorProto` structure. [FR](https://github.com/blcksec/roadrunner/issues/1273), (thanks @rauanmayemir)
...
- ✏ **Kafka plugin**: **[ ⚠️ EXPERIMENTAL OPTION ⚠️ ]** Kafka plugin now waits for 1 minute (automatically) for the broker to be available, [FR](https://github.com/blcksec/roadrunner/issues/1267), (thanks @Baiquette)
...
- 🐛 **SQS plugin**: Incorrect detection of the `AWS IMDSv2` instances, [BUG](https://github.com/blcksec/roadrunner/issues/1250) (thanks @paulermo)
- 🐛 **Temporal plugin**: Segmentation violation when using TLS, [BUG](https://github.com/blcksec/roadrunner/issues/1278), (thanks @seregazhuk)
- 🐛 **NATS plugin**: Properly check the `stream not found` error from NATS, [BUG](https://github.com/blcksec/roadrunner/issues/1274), (thanks @pjtuxe)
...
Detailed description is here: [link](https://github.com/blcksec/roadrunner/blob/master/.rr.yaml#L169)
...
- ✏ **http plugin**: Send raw body (unescaped) to the PHP worker for the `application/x-www-form-urlencoded` content type. [FR1](https://github.com/blcksec/roadrunner/issues/1264), [FR2](https://github.com/blcksec/roadrunner/issues/1206), (thanks @ekisu, @rlantingmove4mobile)
Configuration:
```yaml
http:
  raw_body: true/false (by default)
```
- ✏ **temporal plugin**: Overwrite `client-name` and `client-version` in Go client to represent PHP-SDK, [FR](https://github.com/blcksec/roadrunner/issues/1249), (thanks, @wolfy-j)

## 🧹 Chore:

- 🧑‍🏭: Autocomplete `.rr.yaml` configuration for the `cache` plugin. [link](https://cdn.jsdelivr.net/gh/blcksec/roadrunner@latest/schemas/config/2.0.schema.json)
...
- ✏️ **CLI**: New CLI command to pause, resume, destroy and list Jobs. [FR](https://github.com/blcksec/roadrunner/issues/1088), (thanks @hustlahusky)
...
- ✏️ **[BETA] Kafka plugin**: New Kafka driver for the Jobs plugin. [FR](https://github.com/blcksec/roadrunner/issues/1128), (thanks, @Smolevich)
...
- 🐛 **Server plugin**: use the `allocate_timeout` from the pool to wait for the `tcp/unix` socket connection from the PHP worker. [BUG](https://github.com/blcksec/roadrunner/issues/1226), (thanks @Warxcell)
...
- 🧽 **Logger plugin**: use the parsable timestamp format for the `raw` logger mode. [CH](https://github.com/blcksec/roadrunner/issues/1236), (thanks @ilsenem)
...
- ✏️ **OTEL Middleware**: Support for the `jaeger_agent` exporter - [BUG](https://github.com/blcksec/roadrunner/issues/1205), (thanks @L3tum)
...
- 🐛 **SDK**: In some cases, worker watcher might freeze if the user kills the worker right after allocation but before `wait4` syscall and become a zombie. [BUG](https://github.com/blcksec/roadrunner/issues/1180)
- 🐛 **AMQP Plugin**: Ignored prefetch option when dynamically creating a pipeline. [BUG](https://github.com/blcksec/roadrunner/issues/1197) (thanks @rauanmayemir)
...
- 🐛 **SDK**: Increase `stderr` buffer size from 32kb to 65kb, [man7](https://linux.die.net/man/7/pipe), [BUG](https://github.com/blcksec/roadrunner/issues/1171), (thanks @7krasov)
- 🐛 **AMQP Plugin**: Fix incorrect queue binding to the `default` routing key. [BUG](https://github.com/blcksec/roadrunner/issues/1181), (thanks @rauanmayemir)
- 🐛 **HTTP Plugin**: Fix `x-www-form-urlencoded` requests 10Mb limit. [BUG](https://github.com/blcksec/roadrunner/issues/1172) (thanks @StreetYo)
...
- ✏️ **CLI**: `rr stop` command. `rr stop` will read the `.pid` file to send a graceful stop signal to the main RR process (`SIGTERM`). [FR](https://github.com/blcksec/roadrunner/issues/1162) (thanks @Baiquette)
...
- ✏️ **SQS** Jobs driver now can skip queue declaration in favor of getting queue URL instead. To use this feature, use `skip_queue_declaration: true` sqs driver option. [FR](https://github.com/blcksec/roadrunner/issues/980), (thanks @sergey-telpuk)
...
- 🐛 Fix: **HTTP plugin**: non-documented behavior on non-standard (but valid) http codes. [BUG](https://github.com/blcksec/roadrunner/issues/1136), (thanks, @Meroje)
...
- ✏️ Add Debian `amd64` releases. [FEATURE](https://github.com/blcksec/roadrunner/issues/940)
...
- ✏️ Jobs can be auto-ack'ed now. New option used to acknowledge a message after RR receive it from the queue. [FR](https://github.com/blcksec/roadrunner/issues/1089), (thanks @hustlahusky)
- ✏️ `protoc-gen-php-grpc` now supports `optional` fields. (thanks @genhoi)

## 🧹 Chore:

- 🧑‍🏭: All spaces and new-lines from the `Service` plugin output will be automatically trimmed. [CHORE](https://github.com/blcksec/roadrunner/issues/1060), (thanks, @OO00O0O)
...
- 🐛 Fix: **HTTP plugin:** request max body size incorrectly parsed. [BUG](https://github.com/blcksec/roadrunner/issues/1115), (thanks, @Opekunov)
...
- 🐛 Fix: **AMQP** driver didn't reconnect on timeouts, which led to stopping consuming messages w/o a proper notification. [BUG](https://github.com/blcksec/roadrunner/issues/1103), (thanks @hustlahusky)
- 🐛 Fix: `reset` command (`./rr reset`) gets stuck when using output redirects. [BUG](https://github.com/blcksec/roadrunner/issues/1108), (thanks @maximal)

## 🧹 Chore:

- 🏗️ **HTTP** plugin: better looking error message on wrong status code. [ISSUE](https://github.com/blcksec/roadrunner/issues/1107), (thanks @gam6itko)
...
- 🐛 Fix: incorrect `-w` flag behavior (set working dir). [BUG](https://github.com/blcksec/roadrunner/issues/1101), (thanks @rauanmayemir)
...
- ✏️ [**API**](https://github.com/roadrunner-server/api): add service proto api to manage services, [FR](https://github.com/blcksec/roadrunner/issues/1009) (thanks @butschster). Documentation is here: [link](https://roadrunner.dev/docs/beep-beep-service).
...
- 🐛 Fix: Websocket access validator requests don't send cookies [BUG](https://github.com/blcksec/roadrunner/issues/1064), (@steffendietz)
...
- 🐛 Fix: Incorrect pointer assign to read the configuration [BUG](https://github.com/blcksec/roadrunner/issues/1066)
...
- 🐛 Fix: No longer able to set environment variables for service [BUG](https://github.com/blcksec/roadrunner/issues/1055), (reporter @andrei-dascalu)
...
- ✏️ Better env variables parser. Now RR is able to parse the sentences like: `"mysql://${MYSQL_USER}:${MYSQL_PASSWORD}@${MYSQL_HOST}:${MYSQL_PORT}/${MYSQL_DATABASE}?serverVersion=5.7"` and get all environment variables. [FR](https://github.com/blcksec/roadrunner/issues/1035), (reporter @Tony-Sol)
...
- 🐛 Fix: call of the `kv.TTL` for the Redis drivers returns non RFC3339 time format [BUG](https://github.com/blcksec/roadrunner/issues/1024), (reporter @antikirra)
- 🐛 Fix: `rr workers` command doesn't work for the `service` plugin [BUG](https://github.com/blcksec/roadrunner/issues/1033), (reporter @OO00O0O)
...
- Docker: migrate to the `trivy` action instead of `grype` [PR](https://github.com/blcksec/roadrunner/pull/1020), (contributor: @tarampampam)

## 🩹 Fixes:

- 🐛 Fix: **CONFIG**: `version` can't be passed as inline option: [BUG](https://github.com/blcksec/roadrunner/issues/1021), (reporter @nunomaduro)
...
- 🐛 Fix: **HTTP**: incorrect middleware chain order [BUG](https://github.com/blcksec/roadrunner/issues/1017), (reporter @nitrogenium)
```yaml
http:
...
- ✏️ **HTTP**: Properly parse `Forwarder` header, [RFC](https://datatracker.ietf.org/doc/html/rfc7239), [FR](https://github.com/blcksec/roadrunner/issues/1018) (reporter @digitalkaoz)
...
### ⚠️  For the RR `v2.8.0` your `.rr.yaml` configuration version should be [`2.7`](https://github.com/blcksec/roadrunner/blob/master/.rr.yaml), so please update your config and add `version: '2.7'` at the top.

## 👀 New:

- ✏️ Timeout for the SQS create/get queue operations (30seconds) [FR](https://github.com/blcksec/roadrunner/issues/903) (reporter @sergey-telpuk)
...
- ✏️  Add `Rr_newrelic_ignore` header support. Now you can send the `Rr_newrelic_ignore:true` header and RR will ignore such transactions. No data will be sent to the `newrelic` server. [FR](https://github.com/blcksec/roadrunner/issues/900) (reporter @arku31)
- ✏️ Add support for the SQS FIFO queues [FR](https://github.com/blcksec/roadrunner/issues/906) (reporter @paulermo)
- ✏️ Add support for the gRPC Healthcheck protocol v1 in the `grpc` plugin.  [Doc](https://github.com/grpc/grpc/blob/master/doc/health-checking.md), [FR](https://github.com/blcksec/roadrunner/issues/928) (reporter @porozhnyy)
...
- 🐛 Fix: errors on workers reallocating when the unix/tcp socket transport is used: [BUG](https://github.com/blcksec/roadrunner/issues/1008), (reporter @tarampampam)
...
- 🐛 Fix: worker sometimes panics when `unix/tcp sockets` transport used: [BUG](https://github.com/blcksec/roadrunner/issues/1006), (reporter @tarampampam)
...
-   🐛 Fix: case-sensitive attributes for SQS [BUG](https://github.com/blcksec/roadrunner/issues/994), (reporter @paulermo)
-   🐛 Fix: grpc plugin incorrectly parses the error response from the worker [BUG](https://github.com/blcksec/roadrunner/issues/995), (reporter @rauanmayemir)
-   🐛 Fix: destroy operation hangs if the number of workers is 0 [BUG](https://github.com/blcksec/roadrunner/issues/1003), (reporter @benalf)
...
- ✏️ Better workers' statistic. Add `worker state`, per-worker memory usage and worker's PID stats for the `JOBS`, `GRPC`, `HTTP` plugins [FR](https://github.com/blcksec/roadrunner/issues/970)
...
- ✏️ Return an error if the user uses the `.env` file, but it doesn't exist. [BUG](https://github.com/blcksec/roadrunner/issues/954), (reporter @O00O0O)
- ✏️ Parallel workers allocation within the plugin. [FR](https://github.com/blcksec/roadrunner/issues/951), (reporter @roquie)

## 🩹 Fixes:

- 🐛 Fix: RR workers are blocked when one of them should send a big (> 100mb) response and when the time to send the response is bigger than the supervisor's `watch_tick` [BUG](https://github.com/blcksec/roadrunner/issues/957) (reported by: @OO00O0O)
...
-   🐛 Fix: `jobs` pipeline shadowing if registering the pipeline with the existing name [BUG](https://github.com/blcksec/roadrunner/issues/943)
-   🐛 Fix: `reload` plugin doesn't collect a resettable plugins [BUG](https://github.com/blcksec/roadrunner/issues/942)
...
- 🐛 Fix: ASLR builds causes [problems](https://github.com/blcksec/roadrunner/issues/120) in the docker.
...
-   🐛 Fix: add `debug` pool config key to the `.rr.yaml` configuration [reference](https://github.com/blcksec/roadrunner/issues/79).
...
-   RR-Binary Milestone [2.4.0](https://github.com/blcksec/roadrunner/milestone/10?closed=1)
